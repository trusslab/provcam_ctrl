...
// looks like MB doesn't have (T)RG
#include <stddef.h>
void pseudo_rng(void *p, size_t p_size){
	int r, i;
	for (i = 0; i < p_size; i += sizeof(int))
	{
		// get random int
		r = rand();

		// check if enough space to write
		if ((i + sizeof(int)) > p_size)
			break;

		// write
		memcpy(p + i, &r, sizeof(int));
	}

	// write rest of data
	if (i < p_size)
		memcpy(p + i, &r, p_size - i);
}

#if default_RNG_defined
static uECC_RNG_Function g_rng_function = &default_RNG;
#else
static uECC_RNG_Function g_rng_function = &pseudo_rng;
#endif
...
int uECC_make_key(uint8_t *public_key,
    ...
    for (tries = 0; tries < uECC_RNG_MAX_TRIES; ++tries) {
        if (!uECC_generate_random_int(_private, curve->n, BITS_TO_WORDS(curve->num_n_bits))) {
        	printf("%s: random int generation failed...\n", __func__);
            return 0;
        }
    	...
    	printf("%s: public key cannot be computed at try: %d...\n", __func__, tries);
    }
    return 0;
}
